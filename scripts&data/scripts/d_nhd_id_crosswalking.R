...
# On Sep 29 2023 I started revamping this code to implement Paul Frater's consolidated lake ID crosswalk
...
#scope out the MN crosswalk
laa[state == "Minnesota", .N, lake_id] %>%
  filter(str_detect(lake_id, regex("[A-Za-z]+")) == F) %>% #
  local_to_nhdhr(from_colname = "lake_id") %>% 
  group_by(is.na(nhdhr.id)) %>% 
  summarise(n = n())
...
#scope out the WI crosswalk
laa[state == "Wisconsin", .N, lake_id] %>%
  # filter(str_detect(lake_id, regex("[A-Za-z]+")) == F) %>% #
  local_to_nhdhr(from_colname = "lake_id") %>% 
  group_by(is.na(nhdhr.id)) %>% 
  summarise(n = n())
