...
nhds[ , MNDOW_ID_c := word(MNDOW_ID, -1, sep = "_") ]
...
molten <- melt(nhds[!is.na(MNDOW_ID), .(site_id, MNDOW_ID)], id.vars = "MNDOW_ID", na.rm = TRUE)

any(duplicated(molten))

molten <- molten[!duplicated(molten)]

molten[ , case := seq_len(.N) , .(value, MNDOW_ID) ]

molten[ , variable.v := paste(value,case, sep = ".")]

nhds.mnkey <- dcast(molten[!is.na(value)], MNDOW_ID~variable.v, value.var = "value")
